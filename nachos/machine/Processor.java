...
public final class Processor {
	 ...
	 *
	 * <p>
	 * When the exception handler is called, interrupts will be enabled, and the CPU
	 * cause register will specify the cause of the exception (see the
	 ...
	 *
	 * <p>
	 * If <tt>false</tt>, this processor directly supports single-level paging; use
	 * <tt>setPageTable()</tt>.
	 *
	 * <p>
	 * If <tt>true</tt>, this processor has a software-managed TLB; use
	 * <tt>getTLBSize()</tt>, <tt>readTLBEntry()</tt>, and <tt>writeTLBEntry()</tt>.
	 *
	 * <p>
	 * Using a method associated with the wrong address translation mechanism will
	 ...
	 *
	 * <p>
	 * The TLB is fully associative, so the location of an entry within the TLB does
	 * not affect anything.
	 *
	 * @param number the index into the TLB.
	...
	/**
	 * Read </i>size</i> (1, 2, or 4) bytes of virtual memory at <i>vaddr</i>, and
	 * return the result.
	 *
	 ...
	 * @param nextLoadMask   the mask specifying which bits in the new target are to
	 *                       be overwritten. If a bit in <tt>nextLoadMask</tt> is 0,
	 *                       then the corresponding bit of register
	 ...
	 *
	 * <p>
	 * Transfer the contents of the nextPC register into the PC register, and then
	 * add 4 to the value in the nextPC register. Same as
	 * <tt>advancePC(readRegister(regNextPC)+4)</tt>.
	 *
	 * <p>
	 * Use after handling a syscall exception so that the processor will move on to
	...
	/**
	 * Either an associative or direct-mapped set of translation entries, depending
	 * on whether there is a TLB.
	 */
