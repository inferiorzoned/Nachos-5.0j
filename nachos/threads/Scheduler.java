...
public abstract class Scheduler {
     ...
     * then threads waiting on the new queue will transfer their "priority" to the
     * thread that has access to whatever is being guarded by the queue. This is the
     * mechanism used to partially solve priority inversion.
     *
     * <p>
     * If there is no definite thread that can be said to have "access" (as in the
     * case of semaphores and condition variables), this parameter should be
     ...
     * priority to a thread that already has the processor. When the processor wait
     * queue is created, this parameter should be <tt>false</tt>.
     *
     * <p>
     ...
     * thread holding the lock. Therefore, a queue for a lock should be created with
     * this parameter set to <tt>true</tt>.
     *
     * <p>
     * Similarly, when a thread is asleep in <tt>join()</tt> waiting for the target
     * thread to finish, the sleeping thread should donate its priority to the
     * target thread. Therefore, a join queue should be created with this parameter
     * set to <tt>true</tt>.
     *
     * @param transferPriority <tt>true</tt> if the thread that has access should
     *                         receive priority from the threads that are waiting on
     *                         this queue.
     ...
     *
     * <p>
     * The effective priority of a thread is the priority of a thread after taking
     * into account priority donations.
     *
     * <p>
     * For a priority scheduler, this is the maximum of the thread's priority and
     * the priorities of all other threads waiting for the thread through a lock or
     * a join.
     *
     * <p>
     * For a lottery scheduler, this is the sum of the thread's tickets and the
     * tickets of all other threads waiting for the thread through a lock or a join.
     *
